...
public class MySqlCommand : IDbCommand
{
    ...
    {
        var p = new MySqlParameter { Name = name, Value = value };
        _parameters[name] = p;
        Console.WriteLine($"  [MySQL] Param: {name} = {value}");
    }
    ...
    public void AddParameter(IDbParameter parameter)
    {
        ...
        Console.WriteLine($"  [MySQL] Param: {parameter.Name} = {parameter.Value ?? "null"} " +
    ...
    public void SetTransaction(IDbTransaction transaction)
    {
        ...
        Console.WriteLine("  [MySQL] Command enlisted in transaction.");
    }

    public IDbDataReader ExecuteReader()
    {
        EnsureOpen();
        Console.WriteLine($"  [MySQL] Executing reader: {CommandText}");
    ...
    private void PopulateOutputParameters()
    {
        foreach (var p in _parameters.Values)
        {
            ...
            {
                mp.OutputValue = p.Direction switch
                {
